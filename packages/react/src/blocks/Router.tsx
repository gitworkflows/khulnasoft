...
class RouterComponent extends React.Component<PropsWithChildren<RouterProps>> {
  ...
  private onMouseOverOrTouchStart = (event: MouseEvent | TouchEvent) => {
    ...
    // TODO: use khulnasoft from context
    const attributes = khulnasoft.getUserAttributes();
    ...
    return (
      <KhulnasoftStoreContext.Consumer>
        {state => {
          ...
          return (
            <div className="khulnasoft-router" data-model={model}>
              {/* TODO: move to emotion */}
              <style>{`
                @keyframes khulnasoftLoadingSpinner {
                  0% {
                ...
                .khulnasoft-page-loading {
                  -webkit-animation: khulnasoftLoadingSpinner 1s infinite linear;
                  animation: khulnasoftLoadingSpinner 1s infinite linear;
              ...
              <KhulnasoftComponent
                ...
                modelName={model}
                options={{
                  key: Khulnasoft.isEditing ? undefined : this.model + ':' + url, // TODO: other custom targets specify if should refetch components on change
                }}
              >
                {/* TODO: khulnasoft blocks option for loading stuff */}
                {/* TODO: input for khulnasoft blocks for this */}
                {this.props.children || (
                  <div css={{ display: 'flex' }}>
                    <div css={{ margin: '40vh auto' }} className="khulnasoft-page-loading" />
                  </div>
                )}
              </KhulnasoftComponent>
            </div>
...
export const Router = withKhulnasoft(RouterComponent, {
  name: 'Core:Router',
