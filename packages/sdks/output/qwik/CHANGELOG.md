# Khulnasoft.com Qwik SDK Changelog (@khulnasoft.com/sdk-qwik)
...
- abe5cba: Feat: exports `setClientUserAttributes` helper that can be used to set and update Khulnasoft's user attributes cookie. This cookie is used by Khulnasoft's Personalization Containers to decide which variant to render.
  ...
  ```ts
  import { setClientUserAttributes } from "@khulnasoft.com/sdk-qwik";
...
- 9c4ab57: Feature: add `@khulnasoft.com/sdk-qwik/node/init` entry point with `initializeNodeRuntime` export that sets the IVM instance.
  ...
  import { initializeNodeRuntime } from "@khulnasoft.com/sdk-qwik/node/init";
...
Fix: react and rerender components when Khulnasoft content updates.
