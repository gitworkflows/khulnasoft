...
/**
 * Khulnasoft app-wide page to list and create campaigns
 */
export function CampaignsPage(props: AppTabProps) {
  const state = useLocalStore(() => ({
    campaigns: [] as KhulnasoftContent[],
    teams: [] as KhulnasoftContent[],
    ...
    hasAccessToCampaign(campaign: KhulnasoftContent) {
    ...
    async getTeams() {
      ...
      const teams = await fetch(
        ...
        `https://cdn.khulnasoft.com/api/v3/content/team?apiKey=${user.apiKey}&query.published.$ne=archived&limit=50&cachebust=true`,
        {
    ...
    async getCampaigns() {
      ...
      const campaigns = await fetch(
        // See https://www.khulnasoft.com/c/docs/query-api
        `https://cdn.khulnasoft.com/api/v3/content/campaign?apiKey=${user.apiKey}&query.published.$ne=archived&limit=50&cachebust=true`,
        {
          headers: user.authHeaders,
        }
      ).then((res) => res.json())
