...
const globalLoadedVar = 'KhulnasoftFiddleLoaded';
...
if (isBrowser && !globalVar[globalLoadedVar]) {
  ...
  class KhulnasoftFiddle extends HTMLElement {
    ...
    connectedCallback() {
      ...
      const host = this.getAttribute('host')
        ...
        ? 'http://localhost:1234'
        : qa
        ? 'https://qa.khulnasoft.com'
        : beta
        ? 'https://beta.khulnasoft.com'
      ...
      if (useSpacer) {
        ...
        spacer.className = 'khulnasoft-fiddle-spacer';
    ...
    private onWindowMessage = (event: MessageEvent) => {
      const { data } = event;
      if (data) {
        if (data.type === 'khulnasoft.fiddleLoaded') {
          this.doneLoading();
        }
      }
    };
    ...
    injectStyles() {
      const stylesId = 'khulnasoft-fiddle-styles';
  ...
  customElements.define('khulnasoft-fiddle', KhulnasoftFiddle);
}
