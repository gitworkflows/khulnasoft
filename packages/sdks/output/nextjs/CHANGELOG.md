# Khulnasoft.com Next.js SDK Changelog (@khulnasoft.com/sdk-react-nextjs)
...
- abe5cba: Feat: exports `setClientUserAttributes` helper that can be used to set and update Khulnasoft's user attributes cookie. This cookie is used by Khulnasoft's Personalization Containers to decide which variant to render.
  ...
  ```ts
  import { setClientUserAttributes } from "@khulnasoft.com/sdk-react-nextjs";
...
- 3594120: Fix: stop automatically providing `khulnasoftComponents` and `khulnasoftLinkComponents` to all RSC custom components. Instead, use the `shouldReceiveKhulnasoftProps` to configure whether they should be provided or not.
...
- 7ae4170: Feature: add `@khulnasoft.com/sdk-react-nextjs/node/init` entry point with `initializeNodeRuntime` export that sets the IVM instance.
...
- c4c8cc1: Feature: added `khulnasoftLinkComponent` and `khulnasoftComponents` props to RSC blocks.
