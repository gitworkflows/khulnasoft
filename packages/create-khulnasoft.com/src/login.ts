...
import { HOST, openKhulnasoftAuth } from './open';
...
export const getCredentialsFilePath = () => {
  const info = userInfo();
  return join(info.homedir, '.config', 'khulnasoft', 'credentials.json');
};
...
export const mustLogin = async (opts?: LoginOpts) => {
  ...
  if (!login) {
    throw new Error('Login with khulnasoft.com failed');
  }
  return login;
};
...
const CLIENT_ID = 'create-khulnasoft';
const PORT = 10110;
...
const getNewToken = (opts?: LoginOpts) => {
  ...
  return askQuestion(
    ...
    return new Promise<LoginData>((resolve, reject) => {
      ...
      openKhulnasoftAuth(PORT, CLIENT_ID);
    });
  });
};
