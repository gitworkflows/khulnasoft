...
export const PAGES: Record<string, Page> = {
  ...
  '/editing-empty-content-element-ref': {
    content: null as unknown as KhulnasoftContent,
...
const getContentForPathname = (pathname: string): KhulnasoftContent | null => {
...
type ContentResponse = { results: KhulnasoftContent[] };

export const getProps = async (args: {
  sdk?: Sdk;
  pathname?: string;
  _processContentResult?: (options: any, content: ContentResponse) => Promise<KhulnasoftContent[]>;
  fetchOneEntry?: (opts: any) => Promise<KhulnasoftContent | null>;
  options?: any;
  data?: 'real' | 'mock';
  apiKey?: string;
}) => {
  ...
  switch (pathname) {
    ...
    case '/override-base-url':
      extraProps = {
        apiHost: 'https://cdn-qa.khulnasoft.com',
      };
      break;
