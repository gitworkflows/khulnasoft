...
export const verifyTabContent = async (
  ...
  const visibleContent = page.locator(`[khulnasoft-path="${expectedVisibleContent}"]`);
  await expect(visibleContent).toBeVisible();

  const hiddenContent = page.locator(`[khulnasoft-path="${expectedHiddenContent}"]`);
...
export const getKhulnasoftSessionIdCookie = async ({ context }: { context: BrowserContext }) => {
  const cookies = await context.cookies();
  const khulnasoftSessionCookie = cookies.find(cookie => cookie.name === 'khulnasoftSessionId');
  return khulnasoftSessionCookie;
};
