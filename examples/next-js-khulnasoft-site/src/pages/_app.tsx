...
khulnasoft.allowCustomFonts = false;
...
function MyApp({ Component, pageProps, router }: AppPropsType) {
  if (typeof window !== 'undefined') {
    window.addEventListener('khulnasoft:component:load', () => {
  ...
  const handleClick = useCallback((event: React.MouseEvent<Element>) => {
    ...
    if (
      ...
      anchor?.host.includes('khulnasoft.com') &&
      anchor &&
      (anchor.pathname === '/' ||
  ...
  return (
    <>
      <Head>
        <title>
          Khulnasoft.com: Visual no-code content management for e-comerce
        </title>
        ...
        {!router.asPath.includes('/blog/') && (
          <>
            <meta
              key="og:image"
              property="og:image"
              content="https://cdn.khulnasoft.com/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F1bfe678f91004935b3897c7f3881014b"
            />
            <meta
              key="og:image:width"
        ...
        <link rel="canonical" href={`https://www.khulnasoft.com${urlPath}`} />
        <meta
          key="og:url"
          property="og:url"
          content={`https://www.khulnasoft.com${urlPath}`}
        />
      </Head>
