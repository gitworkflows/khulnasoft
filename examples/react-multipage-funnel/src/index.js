...
import { KhulnasoftComponent, KhulnasoftContent, khulnasoft } from '@khulnasoft.com/react';

import './index.css';

khulnasoft.init('cebf462cf12940c78302d9b4a201d7e6');

function App() {
  return (
    <BrowserRouter>
      <KhulnasoftContent options={{ includeRefs: true }} model="multipage-funnel">
        {(funnel, loading) => {
          if (loading) {
            return 'loading...';
          }
          if (!funnel) {
            return 'no data have you connected khulnasoft correctly?';
          }
