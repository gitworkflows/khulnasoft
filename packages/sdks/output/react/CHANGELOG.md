# Khulnasoft.com React SDK Changelog (@khulnasoft.com/sdk-react)
...
- abe5cba: Feat: exports `setClientUserAttributes` helper that can be used to set and update Khulnasoft's user attributes cookie. This cookie is used by Khulnasoft's Personalization Containers to decide which variant to render.
  ...
  ```ts
  import { setClientUserAttributes } from "@khulnasoft.com/sdk-react";
...
- d8e08ae: Feature: add `@khulnasoft.com/sdk-react/node/init` entry point with `initializeNodeRuntime` export that sets the IVM instance.
  ...
  - Your Remix route's `loader` responsible for fetching the page content from Khulnasoft.com:
