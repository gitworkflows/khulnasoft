import { khulnasoft, KhulnasoftComponent, Image } from '@khulnasoft.com/react';
...
import { footerKhulnasoftEntryId } from '../constants/footer-khulnasoft-entry-id';
...
function Blog({ articles, header, footer, page }: any /* TODO: types */) {
  ...
  return (
    ...
    >
      <Head>
        <title>Khulnasoft.com Blog</title>
        <meta
          name="description"
      ...
      <div css={{ maxWidth: 1200, width: '100%', margin: '40px auto 0' }}>
        <KhulnasoftComponent name="symbol" content={footer} />
      </div>
    </div>
  );
}
...
const getContent = async (context: GetStaticPropsContext) => {
  ...
  const [articles, header, footer] = await Promise.all([
    khulnasoft.getAll('blog-article', {
    ...
    khulnasoft.get('docs-header', { userAttributes: attributes }).promise(),
    khulnasoft
      .get('symbol', {
        userAttributes: { ...attributes },
        entry: footerKhulnasoftEntryId,
      })
      .promise(),
  ]);
