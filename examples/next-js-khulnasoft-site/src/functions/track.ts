...
export const isDev = [
  'localhost',
  'local.khulnasoft.com',
  'local-qa.khulnasoft.com',
].includes(window.location.hostname);

const noTrack = Khulnasoft.isPreviewing || Khulnasoft.isEditing;
...
const userId = getCookie('khulnasoft.userId');
...
const throttledClearInternalQueue = throttle(
  () => {
    ...
    eventQueue = [];
    return fetch('https://khulnasoft.com/api/v1/event', {
      method: 'POST',
...
function addAdditionalScripts() {
  ...
  if (getCookie('khulnasoft.userId')) {
