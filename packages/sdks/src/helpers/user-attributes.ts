...
export const USER_ATTRIBUTES_COOKIE_NAME = 'khulnasoft.userAttributes';
...
if (isBrowser() && TARGET === 'qwik') {
  if (!(window as any).__KHULNASOFT_USER_ATTRIBUTES_SERVICE__) {
    (window as any).__KHULNASOFT_USER_ATTRIBUTES_SERVICE__ =
      createUserAttributesService();
  }
  _userAttributesService = (window as any).__KHULNASOFT_USER_ATTRIBUTES_SERVICE__;
} else {
