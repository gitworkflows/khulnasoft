...
fun getContent(modelName: String, apiKey: String, url: String, onDone: (KhulnasoftContent) -> Unit) {
    val url = "https://cdn.khulnasoft.com/api/v3/content/$modelName?url=$url&apiKey=$apiKey&single=true"
    GlobalScope.launch {
        val responseString = URL(url).readText()
        val obj = Json {
            ignoreUnknownKeys = true
        }.decodeFromString<KhulnasoftContent>(responseString)
        onDone(obj)
    }
}

fun getStyleInt(block: KhulnasoftBlock, property: String): Int {
...
fun getStyleColor(block: KhulnasoftBlock, property: String, default: Color): Color {
...
fun getStyle(block: KhulnasoftBlock, property: String): String? {
