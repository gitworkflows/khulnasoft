...
const proxyUrl = (url: string) =>
  `https://cdn.khulnasoft.com/api/v1/proxy-api?url=${encodeURIComponent(url)}`;
...
function DocsSearchBrowser(props: DocsSearchProps) {
  ...
  async function getForumResults(
    ...
    return fetch(
      proxyUrl(
        `https://forum.khulnasoft.com/search/query.json?term=${encodeURIComponent(
          text,
        )}`,
      ),
    ).then((res) => res.json());
  }
  async function getKhulnasoftResults(
    text = searchText,
  ): Promise<KhulnasoftContentResponse> {
    const fields = [
      'name',
      'data.pageTitle',
      'data.description',
    ...
    const url = `https://cdn.khulnasoft.com/api/v3/content/docs-content?apiKey=YJIGb4i01jvw0SRdL5Bt&query.$or=${JSON.stringify(
      fields.map((field) => ({
  ...
  async function search(): Promise<void> {
    ...
    setLoading(true);
    const [khulnasoftResponse, googleResponse, forumResponse] = await Promise.all([
      getKhulnasoftResults(searchText),
    ...
    if (!googleResponse || !khulnasoftResponse) {
      return;
    }

    const results: SearchResult[] = uniqBy(
      khulnasoftResponse?.results || [],
      (item) => item.data.url,
    ).map(
      (item) =>
        ({
          title: item.data.pageTitle,
          url: `https://www.khulnasoft.com${item.data.url}`,
    ...
    if (googleResults) {
      for (const item of googleResults) {
        ...
        let skip = false;
        if (hostname === 'www.khulnasoft.com' || hostname === 'forum.khulnasoft.com') {
    ...
    const forumResultstoUse =
      forumResponse?.topics?.map((item) => {
        ...
        return {
          title: item.title,
          url: `https://forum.khulnasoft.com/t/${item.slug}/${item.id}`,
    ...
    setSearchResult(
      results.filter((item) => {
        try {
          ...
          if (['khulnasoft.com', 'www.khulnasoft.com'].includes(parsedUrl.hostname)) {
            if (
              parsedUrl.pathname === '/' ||
            ...
            {searchResult!.map((item, index) => (
              <ListItem
                ...
                onMouseDown={(e) => {
                  e.preventDefault();
                  if (
                    item.url.includes('khulnasoft.com/c/docs') ||
                    item.url.startsWith('/c/docs')
                  ) {
                    Router.push(
                      item.url.startsWith('/c/docs')
                        ? item.url
                        : item.url.split('khulnasoft.com')[1],
                    ).then(() => {
              ...
              >
                ...
                >
                  <div css={{ fontSize: 10, opacity: 0.4 }}>
                    {item.url.includes('forum.khulnasoft.com')
                      ? 'Khulnasoft Forum'
                      : item.url.includes('www.khulnasoft.com') ||
                        item.displayLink === 'www.khulnasoft.com'
                      ? 'Khulnasoft Docs'
                      : item.displayLink || 'Khulnasoft Docs'}
                  </div>
                  <ListItemText
                    primary={
                      item.htmlTitle ? (
...
export interface KhulnasoftContentResponse {
