...
import type { KhulnasoftContent } from '../specs/types.js';
...
export const launchEmbedderAndWaitForSdk = async ({
  ...
  if (sdk === 'oldReact') {
    await page.route('https://cdn.khulnasoft.com/api/v3/query/**', async route => {
...
export const cloneContent = (content: KhulnasoftContent) => JSON.parse(JSON.stringify(content));
