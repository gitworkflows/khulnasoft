export const COLUMNS_VERTICAL_CENTERING = {
  ownerId: 'ad30f9a246614faaa6a03374f83554c9',
  lastUpdateBy: null,
  createdDate: 1742468200898,
  id: '34d8d0d89db2442cb3d0b739991fb67f',
  '@version': 4,
  name: 'columns-v-centering',
  modelId: '17c6065109ef4062ba083f5741f4ee6a',
  published: 'draft',
  meta: {
    kind: 'page',
    lastPreviewUrl:
      'http://localhost:5173/columns-v-centering?khulnasoft.space=ad30f9a246614faaa6a03374f83554c9&khulnasoft.user.permissions=read%2Ccreate%2Cpublish%2CeditCode%2CeditDesigns%2Cadmin%2CeditLayouts%2CeditLayers%2CeditContentPriority&khulnasoft.user.role.name=Admin&khulnasoft.user.role.id=admin&khulnasoft.cachebust=true&khulnasoft.preview=page&khulnasoft.noCache=true&khulnasoft.allowTextEdit=true&__khulnasoft_editing__=true&khulnasoft.overrides.page=34d8d0d89db2442cb3d0b739991fb67f&khulnasoft.overrides.34d8d0d89db2442cb3d0b739991fb67f=34d8d0d89db2442cb3d0b739991fb67f&khulnasoft.overrides.page:/columns-v-centering=34d8d0d89db2442cb3d0b739991fb67f&khulnasoft.options.locale=Default',
    hasLinks: false,
  },
  priority: -1015,
  query: [
    {
      '@type': '@khulnasoft.com/core:Query',
      property: 'urlPath',
      operator: 'is',
      value: '/columns-v-centering',
    },
  ],
  data: {
    title: 'columns-v-centering',
    themeId: false,
    blocks: [
      {
        '@type': '@khulnasoft.com/sdk:Element',
        '@version': 2,
        id: 'khulnasoft-f1ae6670480d44bfa78a4a3f15ee584b',
        component: {
          name: 'Columns',
          options: {
            columns: [
              {
                blocks: [
                  {
                    '@type': '@khulnasoft.com/sdk:Element',
                    '@version': 2,
                    id: 'khulnasoft-5f09976df08d4a67b9780dcb38b9ba45',
                    component: {
                      name: 'Text',
                      options: {
                        text: 'text 1',
                      },
                    },
                    responsiveStyles: {
                      large: {
                        display: 'flex',
                        flexDirection: 'column',
                        position: 'relative',
                        flexShrink: '0',
                        boxSizing: 'border-box',
                        marginTop: 'auto',
                        lineHeight: 'normal',
                        height: 'auto',
                        marginBottom: 'auto',
                      },
                    },
                  },
                ],
              },
              {
                blocks: [
                  {
                    '@type': '@khulnasoft.com/sdk:Element',
                    '@version': 2,
                    id: 'khulnasoft-f571ee77a16a45b08f59fdd36463ca5e',
                    component: {
                      name: 'Text',
                      options: {
                        text: 'text 2',
                      },
                    },
                    responsiveStyles: {
                      large: {
                        display: 'flex',
                        flexDirection: 'column',
                        position: 'relative',
                        flexShrink: '0',
                        boxSizing: 'border-box',
                        marginTop: 'auto',
                        lineHeight: 'normal',
                        height: 'auto',
                        marginBottom: 'auto',
                      },
                    },
                  },
                ],
              },
            ],
            space: 20,
            stackColumnsAt: 'tablet',
            reverseColumnsWhenStacked: false,
          },
        },
        responsiveStyles: {
          large: {
            display: 'flex',
            flexDirection: 'column',
            position: 'relative',
            flexShrink: '0',
            boxSizing: 'border-box',
            marginTop: '20px',
            height: '400px',
          },
        },
      },
    ],
  },
  metrics: {
    clicks: 0,
    impressions: 0,
  },
  variations: {},
  lastUpdated: 1742468604796,
  testRatio: 1,
  createdBy: 'RuGeCLr9ryVt1xRazFYc72uWwIK2',
  lastUpdatedBy: 'RuGeCLr9ryVt1xRazFYc72uWwIK2',
  folders: [],
};
