...
useMetadata({
  ...
  angular: {
    selector: 'khulnasoft-content, content-variants',
  },
});
    ...
    <>
      <Show when={!props.isNestedRender && TARGET !== 'reactNative'}>
        <InlinedScript
          ...
          id="khulnasoftio-init-variants-fns"
          nonce={props.nonce || ''}
        />
        {SDKS_SUPPORTING_PERSONALIZATION.includes(TARGET) && (
          <InlinedScript
            ...
            id="khulnasoftio-init-personalization-variants-fns"
          />
        )}
      </Show>
      <Show when={state.shouldRenderVariants}>
        <InlinedStyles
          id="khulnasoftio-variants"
          styles={state.hideVariantsStyleString}
          nonce={props.nonce || ''}
        />
        {/* Sets A/B test cookie for all `RenderContent` to read */}
        <InlinedScript
          id="khulnasoftio-variants-visibility"
