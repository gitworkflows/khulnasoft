...
const SELECTOR = 'div[khulnasoft-content-id]';
...
test.describe('A/B tests', () => {
  test.describe('entire page', () => {
    ...
    const COOKIE_NAME = `khulnasoft.tests.${CONTENT_ID}` as const;
    ...
    for (let i = 1; i <= TRIES; i++) {
      test(`#${i}/${TRIES}: Render default w/ SSR`, async ({
        ...
        await page.route('**/track**', async route => {
          ...
          if (url.includes('cdn.khulnasoft.com/api/v1/track')) {
            trackCalls += 1;
  ...
  test.describe('nested symbol', () => {
    ...
    for (let i = 1; i <= TRIES; i++) {
      test(`#${i}/${TRIES}: Render default w/ SSR`, async ({
        ...
        await expect(page.getByText(TEXTS.DEFAULT_CONTENT).locator('visible=true')).toBeVisible();
        await expect(
          page.locator(SELECTOR + '[khulnasoft-model="symbol"]', { hasText: TEXTS.VARIANT_1 })
        ).toBeHidden();
        await expect(
          page.locator(SELECTOR + '[khulnasoft-model="symbol"]', { hasText: TEXTS.VARIANT_2 })
        ).toBeHidden();
      });
      ...
      test(`#${i}/${TRIES}: Render variant w/ SSR`, async ({
        ...
        await expect(page.getByText(TEXTS.VARIANT_1).locator('visible=true')).toBeVisible();
        await expect(
          page.locator(SELECTOR + '[khulnasoft-model="symbol"]', { hasText: TEXTS.DEFAULT_CONTENT })
        ).toBeHidden();
        await expect(
          page.locator(SELECTOR + '[khulnasoft-model="symbol"]', { hasText: TEXTS.VARIANT_2 })
        ).toBeHidden();
