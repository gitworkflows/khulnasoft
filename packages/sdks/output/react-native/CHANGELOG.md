# Khulnasoft.com React Native SDK Changelog (@khulnasoft.com/sdk-react-native)
...
- abe5cba: Feat: exports `setClientUserAttributes` helper that can be used to set and update Khulnasoft's user attributes cookie. This cookie is used by Khulnasoft's Personalization Containers to decide which variant to render.
  ...
  ```ts
  import { setClientUserAttributes } from "@khulnasoft.com/sdk-react-native";
...
- c787998: Fix: replace `react-native-render-html` with `@khulnasoft.com/react-native-render-html` to remove `defaultProps` errors.
...
- 8cc0cb8: Fix: updates to deeply nested Khulnasoft `state` value now propagate across content.
...
- Use https://github.com/callstack/react-native-khulnasoft-bob to build SDK instead of `tsc`
...
- Fix: Drag'n'Drop in Visual Editor (rendered elements were missing `khulnasoft-id` attribute)

## 0.5.6

- Fix: use `ScrollView` instead of `View` for the `RenderContent` wrapper, to allow for scrolling the Khulnasoft content.
...
- Fix: Khulnasoft blocks no longer add a redundant `Text` wrapper around their `children`.

## 0.1.10

- Fix: Khulnasoft Content blocks are now wrapped in `ScrollView` instead of `View` to allow scrolling.
...
- Fixes `getAllContent` to traverse all nested data from symbols/references https://github.com/khulnasoft-com/khulnasoft/pull/718
- Fixes `getContent` (broken due to missing URL polyfill) https://github.com/khulnasoft-com/khulnasoft/pull/880
- Strips invalid `this.` left from Mitosis compilation https://github.com/khulnasoft-com/khulnasoft/pull/717/commits/b1947c86db769f74a7408965dac70f22dfcc538d
- Fix identification of react-native environment https://github.com/khulnasoft-com/khulnasoft/pull/717/commits/9d2a207ceca39bf83d5bbdc4bd67351e86105d78
- Fix Aspect Ratio handling (this adds support for Pixel Tracking) https://github.com/khulnasoft-com/khulnasoft/pull/749, preceeded by https://github.com/khulnasoft-com/khulnasoft/pull/687
