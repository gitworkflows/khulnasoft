...
let khulnasoftApiKey = 'bb209db71e62412dbe0114bdae18fd15';
let app = express();
...
app.get('*', async (req, res) => {
  let page = await axios
    .get(
      `https://cdn.khulnasoft.com/api/v1/html/page?url=${encodeURI(req.url)}&apiKey=${khulnasoftApiKey}`
    )
    .catch(handleError);

  if (page && page.data) {
    res.send(template({ body: '<h2>This page is from Khulnasoft!</h2>' + page.data.data.html }));
  } else {
    res.send(
      template({
        body: `<h2>No content found :(</h2><p>Have you published a Khulnasoft page for this URL?</p }>`,
      })
    );
  }
});
