...
export function FormBlock(props: FormBlockProps) {
  const { type, khulnasoftState, parameter, customAttributes, khulnasoftBlock, showErrors } = props;
  let form: any = {};
  ...
  // that form object to get it from the Khulnasoft state. https://shopify.dev/docs/themes/liquid/reference/objects/form
  if (Khulnasoft.isBrowser && (window as any).KhulnasoftShopifyData && khulnasoftBlock?.id) {
    const formData = (window as any).KhulnasoftShopifyData[khulnasoftBlock.id]?.form;
  ...
  if (type === 'activate_customer_password') {
    ...
    const product = khulnasoftState?.context.shopify.liquid.get(parameter, khulnasoftState.state);
  ...
  if (customAttributes?.length) {
    customAttributes.forEach(attribute => {
      ...
      if (useValue && khulnasoftState?.state) {
        useValue = khulnasoftState.context.shopify.liquid.render(
          `{{${useValue}}}`,
          ...
          <form {...formAttributes}>
            ...
            {type === 'currency' && (
              <input
                type="hidden"
                name="return_to"
                value={khulnasoftState?.state?.location?.pathname}
              />
            )}
