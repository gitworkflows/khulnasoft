...
const getPrs = async (extra = '') => {
  const prsStr =
    await $`gh search prs ${query} ${extra} --state=open --repo=khulnasoft --owner=KhulnasoftIO --app=dependabot --json=url,number,title`;
  ...
  // exclude `khulnasoft-com/khulnasoft-internal` PRs
  const cleanedPrs = prs.filter(pr => pr.url.includes('khulnasoft-com/khulnasoft/pull'));
  return cleanedPrs;
};
