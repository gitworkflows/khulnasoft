export const SECTION_CHILDREN = {
  ownerId: 'ad30f9a246614faaa6a03374f83554c9',
  lastUpdateBy: null,
  createdDate: 1742361329592,
  id: 'a6c1c51873a94f2d9d74939e3a995bc5',
  '@version': 4,
  name: 'section-children-angular',
  modelId: '17c6065109ef4062ba083f5741f4ee6a',
  published: 'draft',
  meta: {
    kind: 'page',
    lastPreviewUrl:
      'http://localhost:4200/section-children-angular?khulnasoft.space=ad30f9a246614faaa6a03374f83554c9&khulnasoft.user.permissions=read%2Ccreate%2Cpublish%2CeditCode%2CeditDesigns%2Cadmin%2CeditLayouts%2CeditLayers%2CeditContentPriority&khulnasoft.user.role.name=Admin&khulnasoft.user.role.id=admin&khulnasoft.cachebust=true&khulnasoft.preview=page&khulnasoft.noCache=true&khulnasoft.allowTextEdit=true&__khulnasoft_editing__=true&khulnasoft.overrides.page=a6c1c51873a94f2d9d74939e3a995bc5&khulnasoft.overrides.a6c1c51873a94f2d9d74939e3a995bc5=a6c1c51873a94f2d9d74939e3a995bc5&khulnasoft.overrides.page:/section-children-angular=a6c1c51873a94f2d9d74939e3a995bc5&khulnasoft.options.locale=Default',
    hasLinks: false,
  },
  priority: -1010,
  query: [
    {
      '@type': '@khulnasoft.com/core:Query',
      property: 'urlPath',
      operator: 'is',
      value: '/section-children-angular',
    },
  ],
  data: {
    themeId: false,
    title: 'section-children-angular',
    blocks: [
      {
        '@type': '@khulnasoft.com/sdk:Element',
        '@version': 2,
        id: 'khulnasoft-4777cbd5400240fcb5ec3053147350ac',
        component: {
          name: 'Core:Section',
          options: {
            maxWidth: 1200,
            lazyLoad: false,
          },
        },
        children: [
          {
            '@type': '@khulnasoft.com/sdk:Element',
            '@version': 2,
            id: 'khulnasoft-f1ef479d4fc24c3db13f67f5972bbfa4',
            component: {
              name: 'Text',
              options: {
                text: 'text 1',
              },
            },
            responsiveStyles: {
              large: {
                display: 'flex',
                flexDirection: 'column',
                position: 'relative',
                flexShrink: '0',
                boxSizing: 'border-box',
                marginTop: '20px',
                lineHeight: 'normal',
                height: 'auto',
              },
            },
          },
          {
            '@type': '@khulnasoft.com/sdk:Element',
            '@version': 2,
            id: 'khulnasoft-ed79c46cb43848548933175fadb75344',
            component: {
              name: 'Text',
              options: {
                text: 'text 2',
              },
            },
            responsiveStyles: {
              large: {
                display: 'flex',
                flexDirection: 'column',
                position: 'relative',
                flexShrink: '0',
                boxSizing: 'border-box',
                marginTop: '20px',
                lineHeight: 'normal',
                height: 'auto',
              },
            },
          },
        ],
        responsiveStyles: {
          large: {
            display: 'flex',
            flexDirection: 'column',
            position: 'relative',
            flexShrink: '0',
            boxSizing: 'border-box',
            marginTop: '0px',
            paddingLeft: '20px',
            paddingRight: '20px',
            paddingTop: '20px',
            paddingBottom: '20px',
            minHeight: '100px',
          },
        },
      },
    ],
  },
  metrics: {
    clicks: 0,
    impressions: 0,
  },
  variations: {},
  lastUpdated: 1742365828625,
  testRatio: 1,
  createdBy: 'RuGeCLr9ryVt1xRazFYc72uWwIK2',
  lastUpdatedBy: 'RuGeCLr9ryVt1xRazFYc72uWwIK2',
  folders: [],
};
