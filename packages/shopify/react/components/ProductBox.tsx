...
export function ProductBox(props: ProductBoxProps) {
  ...
  function updateProduct() {
    const { context } = props.khulnasoftState!;

    if (productId) {
      setLoading(true);
      fetch(
        `https://cdn.khulnasoft.com/api/v1/shopify/products/${productId}.json?apiKey=${context.apiKey}`
      )
        .then(res => res.json())
        .then(data => {
          const { product } = data;
  ...
  function onClick(e: React.MouseEvent<HTMLDivElement, MouseEvent>) {
    if (!props.khulnasoftBlock?.component?.options?.linkToProductPageOnClick) {
      return;
    }
    if (!productInfo) {
      return;
    }
    if (e.defaultPrevented || Khulnasoft.isEditing) {
      return;
    }
  ...
  return Khulnasoft.isEditing && !productId ? (
    // TODO: <Info> component
    <div
      style={{
        padding: 20,
...
Khulnasoft.registerComponent(ProductBox, {
  ...
  image:
    'https://cdn.khulnasoft.com/api/v1/image/assets%2Fd4fb2d0e6f954b31b60f6f0573e91b0c%2Fff2fd04bad384f30837b40bda8a062dc',
