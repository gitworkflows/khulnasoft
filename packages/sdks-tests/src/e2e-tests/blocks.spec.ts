...
test.describe('Blocks', () => {
  test('Text', async ({ page, sdk, packageName }) => {
    ...
    const textBlocks = page.locator('.khulnasoft-text');
  ...
  test.describe('Image', () => {
    test('Image size', async ({ page, sdk, packageName }) => {
      ...
      await page.route('**/*', route => {
        ...
        if (request.url().includes('cdn.khulnasoft.com/api/v1/image')) {
      ...
      const imageLocator = page.locator('.khulnasoft-image');
    ...
    test("SVG Image shouldn't have srcset", async ({ page, sdk, packageName }) => {
      ...
      const img = page.locator('.khulnasoft-image');
  ...
  test.describe('Video', () => {
    ...
    test('video children', async ({ page, sdk }) => {
      ...
      const noOfVideos = await page.locator('.khulnasoft-video').count();
      ...
      for (let i = 0; i < noOfVideos; i++) {
        ...
        const textBlock = container.locator('.khulnasoft-text p');
  ...
  test.describe('Columns', () => {
        ...
        for (const [columnType, styles] of Object.entries(expected)) {
          test(columnType, async ({ page, sdk }) => {
            ...
            const columns = checkIsRN(sdk)
              ? page.locator('[data-khulnasoft-block-name=khulnasoft-columns]')
              : page.locator('.khulnasoft-columns');
            ...
            const columnLocator = checkIsRN(sdk)
              ? columns.nth(styles.index).locator('[data-khulnasoft-block-name=khulnasoft-column]')
              : columns.nth(styles.index).locator('.khulnasoft-column');
    ...
    test('check different width columns are correctly rendered', async ({ page, sdk }) => {
      ...
      const columns = await page.locator('.khulnasoft-columns').all();
      ...
      for (let i = 0; i < columns.length; i++) {
        ...
        const innerColumns = await columns[i].locator('.khulnasoft-column').all();
    ...
    test('space is correctly allocated', async ({ page, sdk }) => {
      ...
      const columns = page.locator('.khulnasoft-columns').nth(4);

      const firstColumn = columns.locator('.khulnasoft-column').nth(0);
      const secondColumn = columns.locator('.khulnasoft-column').nth(1);
    ...
    test('vertically aligning a block works', async ({ page, sdk }) => {
      ...
      const secondColumn = page.locator('.khulnasoft-column').nth(1);

      const childDiv = secondColumn.locator('div.khulnasoft-blocks');
      ...
      const textBox = await secondColumn.locator('.khulnasoft-text').boundingBox();
    ...
    test('blocks are centered vertically when all blocks are individually centered and columns has a fixed height', async ({
      ...
      const columnsParent = page.locator(
        'xpath=//div[contains(@class, "khulnasoft-columns")]/ancestor::div[1]'
      );

      const columnBox = await columnsParent.boundingBox();
      const textBoxes = await columnsParent.locator('.khulnasoft-text').all();
  ...
  test.describe('Embed', () => {
    test('should render embed', async ({ page, sdk }) => {
      ...
      await expect(page.locator('.khulnasoft-embed iframe')).toBeVisible();

      const iframe = page.locator('.khulnasoft-embed iframe');
  ...
  test.describe('Custom Code', () => {
    test('should render custom code', async ({ page, sdk }) => {
      ...
      await expect(page.locator('.khulnasoft-custom-code iframe')).toBeVisible();

      const iframe = page.locator('.khulnasoft-custom-code iframe');
      const src = await iframe.getAttribute('src');
      expect(src).toContain('https://www.youtube.com/embed/oU3H581uCsA');
    });
