...
function Docs({ docsContent, docsNav, docsHeader }: any /* TODO: types */) {
  ...
  // We do this manually (jquery-esque) so this content can be controlled in Khulnasoft
    ...
    >
      ...
      >
        ...
        >
          ...
          >
            {docsContent || Khulnasoft.isEditing || Khulnasoft.isPreviewing ? (
              <>
                <KhulnasoftComponent
                  renderLink={renderLink}
                  key={docsContent?.id}
...
export const getStaticPaths: GetStaticPaths = async () => {
  const results = await khulnasoft.getAll('docs-content', {
    key: 'articles:all',
    fields: 'data.url',
    limit: 200,
    options: {
...
const getContent = async (context: GetStaticPropsContext) => {
  ...
  const [nav, content, docsHeader] = await Promise.all([
    khulnasoft.get('docs-nav', { userAttributes }).promise(),
    khulnasoft
      .get('docs-content', {
        userAttributes: { ...userAttributes, urlPath: path },
      })
      .promise(),
    khulnasoft.get('docs-header', { userAttributes }).promise(),
  ]);
...
function updateActiveLink(path = location.pathname) {
  const query = `a[href="${path}"],a[href="https://www.khulnasoft.com${path}"],a[href="https://khulnasoft.com${path}"]`;
  const allDocsNavLinksSelector = `[khulnasoft-model="${modelName}"] .${activeLinkClass}`;
