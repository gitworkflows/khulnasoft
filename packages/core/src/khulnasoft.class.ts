...
import { KhulnasoftElement } from './types/element';
...
const sessionStorageKey = 'khulnasoftSessionId';
const localStorageKey = 'khulnasoftVisitorId';
...
type TrackingHook = (
  eventData: Event,
  context: { content?: KhulnasoftContent; [key: string]: any }
) => Event | undefined;
...
interface EventData {
  ...
  targetKhulnasoftElement?: string;
  unique?: boolean;
  metadata?: any | string;
  meta?: any | string;
  sessionId?: string;
 ...
 * https://www.khulnasoft.com/c/docs/guides/targeting-and-scheduling}
 */
...
export type GetContentOptions = AllowEnrich & {
   ...
   *
   * @see {@link https://www.khulnasoft.com/c/docs/query-api#__next:~:text=%26includeRefs%3Dtrue-,cacheSeconds,-No}
   */
  cacheSeconds?: number;
  /**
...
export interface Input {
   ...
   *
   * See all available inputs [here](https://www.khulnasoft.com/c/docs/custom-components-input-types)
...
export interface Component {
   ...
   * Default children
   */
  defaultChildren?: readonly KhulnasoftElement[];
  /**
...
export function KhulnasoftComponent(info: Partial<Component> = {}) {
  return Khulnasoft.Component(info);
}
...
export interface Action {
  ...
  /**
   * Globally unique ID for an action, e.g. "@khulnasoft.com:setState"
   */
  id: string;
}
...
export class Khulnasoft {
  ...
  static singletonInstance: Khulnasoft;
  /**
   * Makes it so that a/b tests generate code like {@link
   * https://www.khulnasoft.com/blog/high-performance-no-code#__next:~:text=Static%20generated%20A%2FB%20testing}
  ...
  static attributesCookieName = 'khulnasoft.userAttributes';
  ...
  static registryChange = new BehaviorSubject({} as typeof Khulnasoft.registry);

  static registerEditor(info: any) {
    if (Khulnasoft.isBrowser) {
      window.postMessage(
        {
          type: 'khulnasoft.registerEditor',
      ...
      if (!Khulnasoft.isTrustedHost(hostname)) {
        console.error(
          'Khulnasoft.registerEditor() called in the wrong environment! You cannot load custom editors from your app, they must be loaded through the Khulnasoft.com app itself. Follow the readme here for more details: https://github.com/khulnasoft-com/khulnasoft/tree/master/plugins/cloudinary or contact chat us in our Spectrum community for help: https://spectrum.chat/khulnasoft'
        );
      }
    }
  ...
  static registerAction(action: Action) {
    ...
    if (Khulnasoft.isBrowser) {
      ...
      window.parent?.postMessage(
        {
          type: 'khulnasoft.registerAction',
          data: actionClone,
        },
        '*'
      );
   ...
   * Khulnasoft.setServerContext(context);
   */
  ...
  static isTrustedHostForEvent(event: MessageEvent) {
    ...
    return url.hostname && Khulnasoft.isTrustedHost(url.hostname);
  }
  ...
  static previewingModel = Khulnasoft.isBrowser && getQueryParam(location.href, 'khulnasoft.preview');
   ...
   * @hidden
   *
   * Use Khulnasoft.register('editor.settings', {}) instead.
   */
  static set(settings: Settings): void {
    Khulnasoft.register('editor.settings', settings);
  }

  static import(packageName: string) {
    if (!Khulnasoft.isBrowser) {
      // TODO: server side support *maybe*
      console.warn('Khulnasoft.import used on the server - this should only be used in the browser');
      return;
    }
    const { System } = window as any;

    if (!System) {
      console.warn('System.js not available. Please include System.js when using Khulnasoft.import');
      return;
    }
    return System.import(`https://cdn.khulnasoft.com/systemjs/${packageName}`);
  }
  ...
  static isEditing = Boolean(
    isIframe &&
      ((document.referrer && document.referrer.match(/khulnasoft\.io|localhost:1234/)) ||
        location.search.indexOf('khulnasoft.frameEditing=') !== -1)
  );

  static isPreviewing = Boolean(
    isBrowser &&
      (location.search.indexOf('khulnasoft.preview=') !== -1 ||
        location.search.indexOf('khulnasoft.frameEditing=') !== -1)
  );
  ...
  static set editingPage(editingPage) {
    ...
    if (isBrowser && isIframe) {
      if (editingPage) {
        document.body.classList.add('khulnasoft-editing-page');
      } else {
        document.body.classList.remove('khulnasoft-editing-page');
      }
    }
  }
  ...
  static registerComponent(component: any, options: Component) {
    ...
    if (editable) {
      ...
      window.parent?.postMessage(
        {
          type: 'khulnasoft.registerComponent',
          data: sendSpec,
        },
        '*'
      );
  ...
  private processEventsQueue() {
    ...
    const fullUserAttributes = {
      ...Khulnasoft.overrideUserAttributes,
  ...
  get browserTrackingDisabled() {
    return Khulnasoft.isBrowser && Boolean((window as any).khulnasoftNoTrack || !navigator.cookieEnabled);
  }
  ...
  private findKhulnasoftParent(target: HTMLElement) {
    return this.findParentElement(target, el => {
      const id = el.getAttribute('khulnasoft-id') || el.id;
      return Boolean(id && id.indexOf('khulnasoft-') === 0);
    });
  }
  ...
  /**
   * Set a hook to modify events being tracked from khulnasoft, such as impressions and clicks
   *
   * For example, to track the model ID of each event associated with content for querying
   * by mode, you can do
   *
   *    khulnasoft.setTrackingHook((event, context) => {
   *      if (context.content) {
  ...
  track(eventName: string, properties: Partial<EventData> = {}, context?: any) {
    ...
    if (isIframe || !isBrowser || Khulnasoft.isPreviewing) {
      return;
    }
    ...
    if (!apiKey) {
      console.error(
        'Khulnasoft integration error: Looks like the Khulnasoft SDK has not been initialized properly (your API key has not been set). Make sure you are calling `khulnasoft.init("«YOUR-API-KEY»");` as early as possible in your application\'s code.'
      );
      return;
    }
  ...
  getSessionId() {
    ...
    try {
      if (Khulnasoft.isBrowser && typeof sessionStorage !== 'undefined') {
  ...
  getVisitorId() {
    ...
    try {
      if (Khulnasoft.isBrowser && typeof localStorage !== 'undefined') {
        // TODO: cookie instead?
  ...
  autoTrack = !Khulnasoft.isBrowser
    ? false
    : !this.isDevelopmentEnv &&
      !(Khulnasoft.isBrowser && location.search.indexOf('khulnasoft.preview=') !== -1);
  ...
  trackInteraction(
    ...
    const targetKhulnasoftElement = target && this.findKhulnasoftParent(target);
    ...
    if (event) {
      ...
      if (targetKhulnasoftElement) {
        const targetRect = targetKhulnasoftElement.getBoundingClientRect();
        const xOffset = clientX - targetRect.left;
        const yOffset = clientY - targetRect.top;
        ...
        metadata.khulnasoftTargetOffset = {
          x: xRatio,
          y: yRatio,
        };
      }
    ...
    const khulnasoftId =
      targetKhulnasoftElement &&
      (targetKhulnasoftElement.getAttribute('khulnasoft-id') || targetKhulnasoftElement.id);

    if (khulnasoftId && targetKhulnasoftElement) {
      metadata.khulnasoftElementIndex = ([] as Element[]).slice
  ...
  constructor(
    ...
    // TODO: use a window variable for this perhaps, e.g. bc webcomponents may be loading khulnasoft twice
    // with it's and react (use rollup build to fix)
    if (Khulnasoft.isBrowser && !forceNewInstance && Khulnasoft.singletonInstance) {
      return Khulnasoft.singletonInstance;
    }
    ...
    if (isBrowser) {
      if (Khulnasoft.isEditing) {
        ...
        parent.postMessage(
          {
            type: 'khulnasoft.animatorOptions',
            data: {
              options: {
                version: 2,
              },
      ...
      // TODO: postmessage to parent the khulnasoft info for every package
      // type: 'khulnasoft.sdk', data: { name: '@khulnasoft.com/react', version: '0.1.23' }
      // (window as any).KHULNASOFT_VERSION = Khulnasoft.VERSION;
      // Ensure always one Khulnasoft global singleton
      // TODO: some people won't want this, e.g. rakuten
      // Maybe hide this behind symbol or on document, etc
      // if ((window as any).Khulnasoft) {
      //   Khulnasoft.components = (window as any).Khulnasoft.components;
      // } else {
      //   (window as any).Khulnasoft = Khulnasoft;
      // }
    }
    ...
    if (isBrowser) {
      ...
      const userAttrCookie = this.getCookie(Khulnasoft.attributesCookieName);
      if (userAttrCookie) {
        try {
  ...
  setTestsFromUrl() {
    ...
    try {
      ...
      const tests = params.khulnasoft && params.khulnasoft.tests;
  ...
  resetOverrides() {
    ...
    // or use khulnasoft.get('foo', { req, res }) in react...........
    Khulnasoft.overrideUserAttributes = {};
  ...
  private bindMessageListeners() {
    addEventListener('message', event => {
      ...
      if (data) {
        switch (data.type) {
          case 'khulnasoft.ping': {
            window.parent?.postMessage(
              {
                type: 'khulnasoft.pong',
                data: {},
              },
              '*'
            );
          ...
          case 'khulnasoft.register': {
            ...
            // TODO: all must have name and can't conflict?
            let typeList = Khulnasoft.registry[type];
            if (!typeList) {
              typeList = Khulnasoft.registry[type] = [];
            }
            typeList.push(info);
            Khulnasoft.registryChange.next(Khulnasoft.registry);
            break;
          }
          case 'khulnasoft.settingsChange': {
            // TODO: possibly do this for all...
            if (event.source === window) {
              break;
            }
            ...
            Object.assign(Khulnasoft.settings, settings);
            Khulnasoft.settingsChange.next(Khulnasoft.settings);
            break;
          }
          case 'khulnasoft.registerEditor': {
            // TODO: possibly do this for all...
            if (event.source === window) {
              break;
            }
            ...
            Khulnasoft.editors.every((thisInfo, index) => {
              if (info.name === thisInfo.name) {
                if (thisInfo.component && !hasComponent) {
                  return false;
                } else {
                  Khulnasoft.editors[index] = thisInfo;
                }
                return false;
              }
              return true;
          ...
          case 'khulnasoft.triggerAnimation': {
            Khulnasoft.animator.triggerAnimation(data.data);
            break;
          }
          ...
          case 'khulnasoft.getComponents':
            window.parent?.postMessage(
              {
                type: 'khulnasoft.components',
                data: Khulnasoft.components.map(item => Khulnasoft.prepareComponentSpecToSend(item)),
              },
              '*'
            );
            break;

          case 'khulnasoft.editingModel':
            this.editingModel = data.data.model;
            break;

          case 'khulnasoft.registerComponent':
            const componentData = data.data;
            Khulnasoft.addComponent(componentData);
            break;

          case 'khulnasoft.blockContentLoading':
            if (typeof data.data.model === 'string') {
              this.blockContentLoading = data.data.model;
            }
            break;

          case 'khulnasoft.editingMode':
            const editingMode = data.data;
            if (editingMode) {
              this.editingMode = true;
              document.body.classList.add('khulnasoft-editing');
            } else {
              this.editingMode = false;
              document.body.classList.remove('khulnasoft-editing');
            }
            break;

          case 'khulnasoft.editingPageMode':
            const editingPageMode = data.data;
            Khulnasoft.editingPage = editingPageMode;
            break;

          case 'khulnasoft.overrideUserAttributes':
            const userAttributes = data.data;
            assign(Khulnasoft.overrideUserAttributes, userAttributes);
            this.flushGetContentQueue(true);
            // TODO: refetch too
            break;

          case 'khulnasoft.overrideTestGroup':
              ...
              if (result && typeof result.then === 'function') {
                (result as Promise<any>)
                  .then(finalResult => {
                    window.parent?.postMessage(
                      {
                        type: 'khulnasoft.evaluateResult',
  ...
  observersByKey: { [key: string]: BehaviorSubject<KhulnasoftContent[]> | undefined } = {};
  ...
  get previewingModel() {
    ...
    return params['khulnasoft.preview'];
  }
  ...
  setUserAttributes(options: object) {
    assign(Khulnasoft.overrideUserAttributes, options);
    if (this.canTrack) {
      this.setCookie(Khulnasoft.attributesCookieName, JSON.stringify(this.getUserAttributes()));
    }
  ...
  get(
    ...
    if (!Khulnasoft.isBrowser) {
      ...
      // directly calls .get on the singleton instance of Khulnasoft
  ...
  queueGetContent(modelName: string, options: GetContentOptions = {}) {
    ...
    const currentObservable = this.observersByKey[key] as BehaviorSubject<KhulnasoftContent[]> | null;
    ...
    if (isEditingThisModel) {
      if (Khulnasoft.isBrowser) {
        parent.postMessage({ type: 'khulnasoft.updateContent', data: { options } }, '*');
      }
    }
    ...
    const observable = new BehaviorSubject<KhulnasoftContent[]>(null as any);
  ...
  get host() {
    switch (this.env) {
      case 'qa':
        return 'https://qa.khulnasoft.com';
      case 'test':
        return 'https://khulnasoft.com-test.web.app';
      case 'fast':
        return 'https://fast.khulnasoft.com';
      case 'cloud':
        return 'https://cloud.khulnasoft.com';
      case 'cdn2':
        return 'https://cdn2.khulnasoft.com';
      case 'cdn-qa':
        return 'https://cdn-qa.khulnasoft.com';
      case 'development':
      case 'dev':
        return 'http://localhost:5000';
      case 'cdn-prod':
        return 'https://cdn.khulnasoft.com';
      default:
        return Khulnasoft.overrideHost || 'https://cdn.khulnasoft.com';
    }
  }

  private getSdkHeaders():
    | {
        'X-Khulnasoft-SDK': string;
        'X-Khulnasoft-SDK-GEN': '1';
        'X-Khulnasoft-SDK-Version': string;
      }
    | {} {
    if (!Khulnasoft.sdkInfo) {
      return {};
    }

    return {
      'X-Khulnasoft-SDK': Khulnasoft.sdkInfo.name,
      'X-Khulnasoft-SDK-GEN': '1',
      'X-Khulnasoft-SDK-Version': Khulnasoft.sdkInfo.version,
    } as const;
  }
  ...
  private flushGetContentQueue(usePastQueue = false, useQueue?: GetContentOptions[]) {
    ...
    const cachebust =
      ...
      pageQueryParams['khulnasoft.cachebust'];
      ...
      .then(
        result => {
          for (const options of queue) {
            ...
            if (isEditingThisModel && Khulnasoft.isEditing) {
              parent.postMessage({ type: 'khulnasoft.updateContent', data: { options } }, '*');
              // return;
            }
            ...
            if (data) {
              const testModifiedResults = Khulnasoft.isServer
                ? sorted
                : this.processResultsForTests(sorted);
              observer.next(testModifiedResults);
            } else {
              const search = this.getLocation().search;
              if ((search || '').includes('khulnasoft.preview=' + options.model)) {
                const previewData = {
                  id: 'preview',
                  name: 'Preview',
                  data: {},
  ...
  private testCookiePrefix = 'khulnasoft.tests';

  private processResultsForTests(results: KhulnasoftContent[]) {
    ...
    if (isIframe) {
      window.parent?.postMessage(
        { type: 'khulnasoft.contentResults', data: { results: mappedResults } },
        '*'
      );
    }
  ...
  getCookie(name: string): any {
    ...
    return Khulnasoft.isBrowser && getCookie(name);
  }

  setCookie(name: string, value: any, expires?: Date) {
    if (this.cookies && !(Khulnasoft.isServer && Khulnasoft.isStatic)) {
      return this.cookies.set(name, value, {
        expires,
        secure: this.getLocation().protocol === 'https:',
      });
    }
    return Khulnasoft.isBrowser && setCookie(name, value, expires);
  }
