  ...
  test.describe('Style Bindings', () => {
    test('Content', async ({ page, sdk }) => {
      ...
      const FIRST_BLOCK_SELECTOR = checkIsRN(sdk)
        ? // ScrollView adds an extra div wrapper
          `${getClassSelector('khulnasoft-blocks', sdk)} > div > div > div > div > div`
        : sdk === 'angular'
          ? `div[khulnasoft-id="khulnasoft-1098ca09970149b3bc4cd43643bd0545"]`
          : `${getClassSelector('khulnasoft-blocks', sdk)} > div`;
    ...
    test('Symbol', async ({ page, sdk }) => {
      ...
      const FIRST_BLOCK_SYMBOL_SELECTOR = checkIsRN(sdk)
        ? `${getClassSelector('khulnasoft-blocks', sdk)} > div > div > div`
        : `${getClassSelector('khulnasoft-blocks', sdk)} > div`;
