...
import { khulnasoft, Khulnasoft, KhulnasoftContent, KhulnasoftContentVariation } from '@khulnasoft.com/sdk';

function getData(content: KhulnasoftContentVariation) {
...
const variantsScript = (variantsString: string, contentId: string) =>
  `
(function() {
  if (window.khulnasoftNoTrack || !navigator.cookieEnabled) {
    return;
  }
  ...
  var cookieName = 'khulnasoft.tests.${contentId}';
...
interface VariantsProviderProps {
  initialContent: KhulnasoftContent;
  children: (variants: KhulnasoftContent[], renderScript?: () => JSX.Element) => JSX.Element;
}

export const VariantsProvider = ({ initialContent, children }: VariantsProviderProps) => {
  if (Khulnasoft.isBrowser && !khulnasoft.canTrack) {
  ...
  const variants: KhulnasoftContent[] = Object.keys(initialContent.variations!).map(id => ({
    id,
  ...
  const cookieName = `khulnasoft.tests.${initialContent.id}`;

  let variantId = khulnasoft.getCookie(cookieName);

  if (!variantId && Khulnasoft.isBrowser) {
    let n = 0;
    ...
    for (let i = 0; i < variants.length; i++) {
      ...
      n += testRatio!;
      if (random < n) {
        khulnasoft.setCookie(cookieName, variant.id);
        variantId = variant.id;
        break;
      }
    }
  ...
  if (!variantId) {
    ...
    khulnasoft.setCookie(cookieName, variantId);
  }
