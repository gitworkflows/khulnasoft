# Khulnasoft.com Vue SDK Changelog (@khulnasoft.com/sdk-vue)
...
- abe5cba: Feat: exports `setClientUserAttributes` helper that can be used to set and update Khulnasoft's user attributes cookie. This cookie is used by Khulnasoft's Personalization Containers to decide which variant to render.
  ...
  ```ts
  import { setClientUserAttributes } from "@khulnasoft.com/sdk-vue";
...
- ff58662: Fix: remove `@khulnasoft.com/sdk-vue/nuxt-initialize-node-runtime-plugin` plugin export

## 2.0.17

### Patch Changes

- 01d8496: Feature: add `@khulnasoft.com/sdk-vue/node/init` entry point with `initializeNodeRuntime` export that sets the `isolated-vm` instance.

  This import should be called in a server-only environment. For convenience, we offer:

  - (Recommended) Updates to `@khulnasoft.com/sdk-vue/nuxt` Nuxt module which helps you achieve this in one place:

    - added `includeCompiledCss` flag that adds the compiled Khulnasoft.com CSS in Nuxt (defaults to `true`)
    ...
    export default defineNuxtConfig({
      ...
      modules: [
        [
          "@khulnasoft.com/sdk-vue/nuxt",
          {
            includeCompiledCss: true, // Includes Khulnasoft.com CSS (default: true)
  ...
  - If you prefer to call this manually without using our Nuxt module, you can import and call `initializeNodeRuntime()` from the package `@khulnasoft.com/sdk-vue/node/init`. Make sure that this function is imported and executed only in the Node runtime environment, for example:

    ```ts
    // server/middleware/khulnasoft.global.ts
...
- 84cd444: feature: add the Khulnasoft Tabs block (ported from gen1 widgets).
...
- cde7c61: feat: export `KhulnasoftContext` from sdks
...
- 9b71eab: Feature: added support for the Khulnasoft `Slot` block
...
- 70fa50d: - 🧨 Breaking: removed Vue 2 SDK
  ...
  - 🧨 Breaking: removed `@khulnasoft.com/sdk-vue/vue3` import. To import the SDK, you should now use:

  ```ts
  // BEFORE
  import { Content } from "@khulnasoft.com/sdk-vue/vue3";

  // AFTER
  import { Content } from "@khulnasoft.com/sdk-vue";
  ```
...
- 🧨 Breaking Changes: remove the CSS stylesheet exports:
  ...
  - `import '@khulnasoft.com/sdk-vue/vue2/css'`
  - `import '@khulnasoft.com/sdk-vue/vue3/css'`
...
- Make the default import (`import Sdk from '@khulnasoft.com/sdk-vue'`) point to `vue3` sub-export instead of `vue2`.
...
- Fix: tracking URL from `khulnasoft.com/api/v1/track` to `cdn.khulnasoft.com/api/v1/track` for improved reliability.
...
- Fix: bring back `getKhulnasoftSearchParams` export that was accidentally removed.
...
In case you feel the need to use our older API Version `v2`, reach out to us at support@khulnasoft.com first. But you can override the default by setting `apiVersion` explicitly to `v2` as follows:
...
- Feature: `RenderContent` and `getContent` now have an `apiVersion` field that can be set to `v2` or `v3` to target specific Khulnasoft API versions. Current default is `v2`
...
- Fixed binding of element events https://github.com/khulnasoft-com/khulnasoft/pull/1767
...
- Feature: support mutating Khulnasoft's `state` in custom element events and custom user code.
...
```ts
// imports Vue 2 SDK
import * as KhulnasoftSDK from "@khulnasoft.com/sdk-vue/vue2";
// fallback to Vue 2 SDK
import * as KhulnasoftSDK from "@khulnasoft.com/sdk-vue";

// imports Vue 3 SDK
import * as KhulnasoftSDK from "@khulnasoft.com/sdk-vue/vue3";
```
...
See [khulnasoft-registered-components.ts](/packages/sdks/src/constants/khulnasoft-registered-components.ts) for examples of how to do so, or see the example provided for this SDK.

## 0.0.1-51

⚠️ Deprecation notice: Registering components via `registerComponent(component, info)` is now deprecated.
To register your custom components in Khulnasoft, you must now provide a `customComponents` array to the `RenderContent` component containing `[{ component, info }]`.
See [khulnasoft-registered-components.ts](/packages/sdks/src/constants/khulnasoft-registered-components.ts) for examples of how to do so, or see the example provided for this SDK.

## 0.0.1-50

- feat: 🎸 export `isPreviewing()` (https://github.com/khulnasoft-com/khulnasoft/pull/951)
- feat: 🎸 Add support for Symbols (https://github.com/khulnasoft-com/khulnasoft/pull/951)
- feat: 🎸 Add support for Data Bindings https://github.com/khulnasoft-com/khulnasoft/pull/970

- BREAKING CHANGE: 🧨 RenderContent must now be provided the `apiKey` as a prop (https://github.com/khulnasoft-com/khulnasoft/pull/951)

## 0.0.1-49

- Fix: show the "+ add block" button on empty pages https://github.com/khulnasoft-com/khulnasoft/pull/934
- Add `getKhulnasoftSearchParams` helper export to easily view current drafts on your production site. https://github.com/khulnasoft-com/khulnasoft/pull/883

## 0.0.1-48

Changes:

- Adds support for Columns https://github.com/khulnasoft-com/khulnasoft/pull/717
- Add preliminary support for Children within custom components https://github.com/khulnasoft-com/khulnasoft/pull/753
- Seeds classnames to reduce variation in changes https://github.com/khulnasoft-com/khulnasoft/pull/703
- Fixes `getAllContent` to traverse all symbols/references https://github.com/khulnasoft-com/khulnasoft/pull/718
