...
/**
 * This script uses the octokit SDK to approve & merge all open dependabot PRs for this khulnasoft-com/khulnasoft repo.
...
console.log('Welcome to the khulnasoft-com/khulnasoft dependabot PR merger!');
...
const getPRs = async ({ isApproved }) => {
  const getQuery = isApproved =>
    `repo:khulnasoft-com/khulnasoft is:open state:open author:app/dependabot type:pr ${query} in:title `;
...
async function workflowStuff() {
  let x = null;
  octokit.actions
    .listRepoWorkflowRuns({ owner: 'KhulnasoftIO', repo: 'khulnasoft', branch: 'main' })
    .then(k => (x = k));

  const cancelWorkflowRun2 = async workflow => {
    try {
      await octokit.request('POST /repos/khulnasoft-com/khulnasoft/actions/runs/{run_id}/cancel', {
        run_id: workflow.id,
      });
  ...
  octokit
    .request('GET /repos/khulnasoft-com/khulnasoft/actions/runs?status=queued&branch=main&per_page=10')
