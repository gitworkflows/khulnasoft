...
const initializeUserAttributes = async (
  ...
  const context = await createContextWithCookies({
    baseURL,
    browser,
    cookies: [{ name: 'khulnasoft.userAttributes', value: JSON.stringify(userAttributes) }],
  });
...
test.describe('Personalization Container', () => {
  ...
  test('setClientUserAttributes and khulnasoft.setUserAttributes sets cookie and renders variant after the first render', async ({
    page,
    packageName,
  }) => {
  ...
  test('root style attribute is correctly set', async ({ page }) => {
    ...
    const secondPersonalizationContainer = page
      .locator('.khulnasoft-personalization-container')
      .nth(1);
