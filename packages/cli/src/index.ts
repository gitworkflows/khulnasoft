...
console.log(chalk.blueBright(figlet.textSync('Khulnasoft.com cli', { horizontalLayout: 'full' })));

program
  .command('import')
  .description('Import a khulnasoft space to the local file system')
  .option('-k,--key <key>', 'Private Key')
  .option('-d,--debug', 'print debugging information')
  .option('-o,--output <output>', 'Path to folder default to ./khulnasoft', './khulnasoft')
...
program
  ...
  .option('-d,--debug', 'print debugging information')
  .option('-i,--input <input>', 'Path to folder default to ./khulnasoft', './khulnasoft')
...
program
  .command('integrate')
  .description('integrate Khulnasoft.com with an existing codebase, currently supports Next.js')
  .option('-d,--debug', 'print debugging information')
  .option('--skip-install', 'skip installing the @khulnasoft.com/react sdk')
  .option('-s,--stack <stack>', 'currently supports nextjs', 'nextjs')
  .option('-m,--model <model>', 'name of the model you want to integrate')
  .option('-a,--apiKey <apiKey>', 'you can find your apiKey on khulnasoft.com/account/settings')
  .option(
    '-c,--content <contentId>',
    'opens the content entry in khulnasoft.com after integration has completed'
  )
  .option(
    '-p,--pathPrefix <prefix>',
