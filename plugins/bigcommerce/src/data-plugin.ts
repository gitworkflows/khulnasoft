import { APIOperations, ResourceType } from '@khulnasoft.com/data-plugin-tools';
import { CommerceAPIOperations } from '@khulnasoft.com/commerce-plugin-tools';
...
function buildBigCommerceUrl({ resource, resourceId, query, limit, headers }) {
  const base = `https://cdn.khulnasoft.com/api/v1/proxy-api?url=https%3A%2F%2Fapi.bigcommerce.com%2Fstores%2Fpdfuqrrtbk%2Fv3%2Fcatalog`;
  let path = resource;
  if (path === 'product') {
    path = 'products';
...
export const getDataConfig = (service: CommerceAPIOperations, headers: any): DataPluginConfig => {
  return {
    name: 'BigCommerce',
    icon: 'https://iili.io/JnTyc4s.png', //bigcommerce logo png, should be replaced with a png hosted by khulnasoft
