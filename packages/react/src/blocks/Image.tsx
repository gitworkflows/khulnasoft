...
class ImageComponent extends React.Component<any, { imageLoaded: boolean; load: boolean }> {
  get useLazyLoading() {
    ...
    // Use khulnasoft.getLocation()
    return Khulnasoft.isBrowser && location.search.includes('khulnasoft.lazyLoadImages=false')
      ? false
      : Khulnasoft.isBrowser && location.href.includes('khulnasoft.lazyLoadImages=true')
        ? true
        : this.props.lazy;
  }
  ...
  componentDidMount() {
    if (this.props.lazy && Khulnasoft.isBrowser) {
  ...
  getSrcSet(): string | undefined {
    ...
    // We can auto add srcset for cdn.khulnasoft.com and shopify
    // images, otherwise you can supply this prop manually
    if (!(url.match(/khulnasoft\.io/) || url.match(/cdn\.shopify\.com/))) {
      return;
    }
  ...
  render() {
    const { aspectRatio, lazy, khulnasoftBlock, khulnasoftState } = this.props;
    const children = this.props.khulnasoftBlock && this.props.khulnasoftBlock.children;

    let srcset = this.props.srcset;
    const sizes = getSizes(
      this.props.sizes,
      khulnasoftBlock,
      khulnasoftState?.context.khulnasoftContent?.meta?.breakpoints || {}
    );
    const image = this.image;

    if (srcset && image && image.includes('khulnasoft.com/api/v1/image')) {
    ...
    const isPixel = khulnasoftBlock?.id.startsWith('khulnasoft-pixel-');
        ...
        {value => {
          ...
          const imageContents = (!lazy || this.state.load || amp) && (
            <Tag
              ...
              className={'khulnasoft-image' + (this.props.className ? ' ' + this.props.className : '')}
              src={this.image}
              {...(!amp && {
            ...
            <React.Fragment>
              {amp ? (
                ...
                <picture ref={ref => (this.pictureRef = ref)}>
                  {srcset && srcset.match(/khulnasoft\.io/) && !this.props.noWebp && (
              ...
              {aspectRatio && !amp && !(fitContent && children && children.length) ? (
                <div
                  className="khulnasoft-image-sizer"
                  css={{
                    width: '100%',
              ...
              {children && children.length ? (
                fitContent ? (
                  children.map((block: KhulnasoftElement, index: number) => (
...
export const Image = withKhulnasoft(ImageComponent, {
  name: 'Image',
  static: true,
  image:
  ...
  inputs: [
    {
      ...
      required: true,
      defaultValue:
        'https://cdn.khulnasoft.com/api/v1/image/assets%2FYJIGb4i01jvw0SRdL5Bt%2F72c80f114dc149019051b6852a9e3b7a',
