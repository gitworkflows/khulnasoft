...
const KHULNASOFT_IO_PERSONALIZATION_SCRIPT_FN_NAME = 'khulnasoftIoPersonalization';
...
export function getBlocksToRender({
  ...
  fallbackBlocks?: KhulnasoftBlock[];
...
export const getInitPersonalizationVariantsFnsScriptString = () => {
  ...
  window.${KHULNASOFT_IO_PERSONALIZATION_SCRIPT_FN_NAME} = ${PERSONALIZATION_SCRIPT}
...
export const getPersonalizationScript = (
  ...
  return `window.${KHULNASOFT_IO_PERSONALIZATION_SCRIPT_FN_NAME}(${JSON.stringify(variants)}, "${blockId}", ${isHydrationTarget}${locale ? `, "${locale}"` : ''})`;
};
