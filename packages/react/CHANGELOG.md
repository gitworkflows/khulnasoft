...
- 58fb07e: Fix: ability to set `khulnasoft.canTrack` before calling `khulnasoft.init()`
- Updated dependencies [58fb07e]
  - @khulnasoft.com/sdk@6.0.4
...
- Updated dependencies [b1ad88c]
  - @khulnasoft.com/sdk@6.0.3

## 8.0.5

### Patch Changes

- 37c9341: fix: add khulnasoft block id only if its present
...
- 56f9461: - Adds `apiEndpoint` prop to `khulnasoft` instance with permitted values being `'content'` or `'query'`. It dictates which API endpoint is used for fetching Khulnasoft content
  - Breaking Change 🧨: Removes `apiEndpoint` argument from `khulnasoft.get()`, `khulnasoft.getAll()`, and the `options` prop of `<KhulnasoftContent>` component. NOTE: this argument was not working as expected.
...
- f12f43d: Breaking Change: Use `/query` instead of `/content` for API calls. This change fixes a symbol rendering issue introduced in https://github.com/khulnasoft-com/khulnasoft/pull/3681, which was included in the 6.0.0 release.

### Patch Changes

- Updated dependencies [f12f43d]
  - @khulnasoft.com/sdk@5.0.0
...
- Updated dependencies [cf33d45]
  - @khulnasoft.com/sdk@4.0.1

## 6.0.0

### Major Changes

- f4fffe9: Permanently removes the `apiEndpoint` prop from `khulnasoft.get()` and `khulnasoft.getAll()` which had options `'content'` and `'query'`. Content API is now the only possible API endpoint for content fetching.

### Patch Changes

- Updated dependencies [f4fffe9]
- Updated dependencies [443a3e3]
  - @khulnasoft.com/sdk@4.0.0
...
- 54af3bb: Fix: previewing SDK content within the Studio tab of the Khulnasoft Visual Editor.
- Updated dependencies [2ae3cc5]
  - @khulnasoft.com/sdk@3.0.5
...
- Updated dependencies [49d0aa3]
  - @khulnasoft.com/sdk@3.0.4

## 5.0.6

### Patch Changes

- d403fca: Adds `apiEndpoint` prop to `khulnasoft.get()` and `khulnasoft.getAll()` with options `'content'` and `'query'`. It dictates which API endpoint is used for the content fetching.
  Defaults to `'query'`
- Updated dependencies [d403fca]
  - @khulnasoft.com/sdk@3.0.3
...
- bba43e6: This introduces two new custom events to enhance tracking and analytics for personalization container variants:
  ...
  1. `khulnasoft.variantLoaded`: Fired when a variant is loaded.
  2. `khulnasoft.variantDisplayed`: Fired when a variant becomes visible in the viewport.

  ### Changes

  - Added `khulnasoft.variantLoaded` event dispatch when a variant is loaded.
  - Implemented an Intersection Observer to trigger the `khulnasoft.variantDisplayed` event when the variant enters the viewport.
  ...
  ```javascript
  document.addEventListener("khulnasoft.variantLoaded", (event) => {
  ...
  document.addEventListener("khulnasoft.variantDisplayed", (event) => {
...
- Updated dependencies [1586519]
  - @khulnasoft.com/sdk@3.0.0

## 4.0.4

### Patch Changes

- b7c00cf: Fix SSR hydration issues with non-hover animated khulnasoft blocks
- Updated dependencies [b7c00cf]
  - @khulnasoft.com/sdk@2.2.9
...
- Add support for `customComponents` prop on `KhulnasoftComponent`
...
- Allow `khulnasoft.get` to be awaited: https://github.com/khulnasoft-com/khulnasoft/pull/2512
...
- Chore: update `@khulnasoft.com/core` to `^2.0.4` to fix import issue
...
In case you feel the need to use our older API Version `v1`, reach out to us at support@khulnasoft.com first. But you can override the default by setting `apiVersion` explicitly to `v1` as follows:

```js
import { khulnasoft } from "@khulnasoft.com/react";

khulnasoft.init("YOUR_KHULNASOFT_PUBLIC_KEY");
khulnasoft.apiVersion = "v1";
```

More details on the Khulnasoft API Versions visit [this link](https://www.khulnasoft.com/c/docs/content-api-versions).

## 2.0.17

- Add new `apiVersion` property to toggle between Khulnasoft API versions. Defaults to `v1`. Possible values: `v1` and `v3`.

You can set the apiVersion using `khulnasoft.init` or `khulnasoft.apiVersion`:
...
- Add support for locale prop on `KhulnasoftComponent` to auto-resolve localized inputs.
- Use the latest core sdk version which addresss an issue with rendering in a middleware (also a shopify hydrogen issue, https://github.com/khulnasoft-com/khulnasoft/issues/610).

## 2.0.4

- updates the patching logic to fix bug while where styling updates in the khulnasoft editor don’t properly apply (will quickly revert, until you refresh the preview)

## 2.0.3

- Fix an issue with previewing drafts of a published data model rendered by `KhulnasoftContent`.
- Fix an issue with live editing on a `KhulnasoftContent` containing a `KhulnasoftComponent` of the same model.
...
- Add new hook `useIsPreviewing` to be used instead of `Khulnasoft.isEditing` and `Khulnasoft.isPreviewing` flags, to fix hydration warnings while editing or previewing.
