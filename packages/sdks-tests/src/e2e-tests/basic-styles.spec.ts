...
test.describe('Basic styles', () => {
  ...
  test('Box with flex-direction row should have children aligned horizontally', async ({
    ...
    const khulnasoftBoxChildren = page
      .locator('[khulnasoft-id="khulnasoft-258a89336ec44f37a38a146e698a858a"]')
      .locator('div[khulnasoft-id]');

    await expect(khulnasoftBoxChildren).toHaveCount(3);

    const firstChild = khulnasoftBoxChildren.nth(0);
    const secondChild = khulnasoftBoxChildren.nth(1);
    const thirdChild = khulnasoftBoxChildren.nth(2);
