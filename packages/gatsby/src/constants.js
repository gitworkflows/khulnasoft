...
export const defaultOptions = {
  ...
  fieldName: `allKhulnasoftModels`,
  typeName: `khulnasoft`,
  limit: 30,
  baseURL: `https://cdn.khulnasoft.com/api/v3/graphql`,
  overrideDev404: true,
  useCache: false,
  batch: true,
  // custom404Dev: 'path to custom 404'
  // globalContext: { store: process.env.STORE_TOKEN } // helpful in multi stores repo
  // filter: (entry) => entry.content.data.store === process.env.STORE_TOKEN // helpful in multi-store khulnasoft organization
};
...
export function getConfig(options) {
  ...
  invariant(
    ...
    `@khulnasoft.com/gatsby requires a public API Key`
  );

  invariant(config.limit < 101, '@khulnasoft.com/gatsby maximum pagination limit is 100');
