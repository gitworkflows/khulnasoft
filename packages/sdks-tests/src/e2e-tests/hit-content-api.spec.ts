...
test.describe('Get Content', () => {
  test('call content API only once - in page', async ({ page, sdk }) => {
    ...
    const urlMatch = /https:\/\/cdn\.khulnasoft\.io\/api\/v3\/content/;
  ...
  test('passes fetch options', async ({ page, packageName }) => {
    ...
    const urlMatch = /https:\/\/cdn\.khulnasoft\.io\/api\/v3\/query/;
  ...
  test('fetch symbol with query.id', async ({ page, sdk, packageName }) => {
    ...
    const urlMatch = /https:\/\/cdn\.khulnasoft\.io\/api\/v3\/content\/symbol/;
  ...
  test('should NOT omit componentsUsed when omit parameter is explicitly set to empty string', async ({
    ...
    let khulnasoftRequestPromise: Promise<string> | undefined = undefined;
    let requestUrl: string | undefined;

    const khulnasoftApiRegex = /https:\/\/cdn\.khulnasoft\.io\/api\/v3\//;

    await page.goto('/get-content-with-omit');

    khulnasoftRequestPromise = new Promise<string>(resolve => {
      page.on('request', request => {
        const url = request.url();
        if (khulnasoftApiRegex.test(url)) {
          requestUrl = url;
          resolve(url);
        }
      });
    ...
    await khulnasoftRequestPromise;
