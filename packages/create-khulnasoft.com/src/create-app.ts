...
import { didOpenBroswer, getEditorURL, openKhulnasoft } from './open';
...
`);
  ...
  if (autoRun) {
    const next = didOpenBroswer()
      ? true
      : await askQuestion(
          `Run dev server and open the khulnasoft's editor in the browser. ${bold('Confirm?')}`
        );
...
`);
      if (!didOpenBroswer()) {
        setTimeout(() => {
          openKhulnasoft(projectName, starter, 3000);
        }, 2000);
      }
...
async function prepare(starter: Starter) {
  ...
  const tmpPath = join(baseDir, '.tmp-khulnasoft-starter');
  ...
  return async (projectName: string, context: { [context: string]: string }) => {
    ...
    for (const [key, value] of Object.entries(context)) {
      await replaceInFile({
        ...
        from: new RegExp(`khulnasoft-${key}`, 'g'),
