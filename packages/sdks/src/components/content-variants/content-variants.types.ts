...
export interface ContentVariantsPrps extends ExtraFrameworkProps {
  /**
   * The Khulnasoft content JSON to render (required).
   */
  content: Nullable<KhulnasoftContent>;

  /**
   * The Khulnasoft content `model` to render (required).
   */
  model: string;

  /**
   * Additional data to inject into your Khulnasoft content (optional).
   */
  data?: { [key: string]: any };
  ...
  /**
   * Sets the host of Khulnasoft API calls. (Defaults to global `https://cdn.khulnasoft.com`)
   */
  apiHost?: string;
