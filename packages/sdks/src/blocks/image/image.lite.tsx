...
import type { JSX } from '@khulnasoft.com/mitosis/jsx-runtime';
  ...
  const state = useStore({
    get srcSetToUse(): string | undefined {
      ...
      if (
        ...
        !(
          typeof url === 'string' &&
          (url.match(/khulnasoft\.io/) || url.match(/cdn\.shopify\.com/))
        )
      ) {
        return props.srcset;
      }
      ...
      if (props.srcset && props.image?.includes('khulnasoft.com/api/v1/image')) {
    ...
    get webpSrcSet() {
      if (state.srcSetToUse?.match(/khulnasoft\.io/) && !props.noWebp) {
    ...
    <>
      <picture>
        ...
        <img
          ...
          class={
            'khulnasoft-image' + (props.className ? ' ' + props.className : '')
          }
          src={props.image}
      ...
      <Show
        when={
          props.aspectRatio &&
          !(props.khulnasoftBlock?.children?.length && props.fitContent)
        }
      >
        <div
      ...
      <Show when={props.khulnasoftBlock?.children?.length && props.fitContent}>
        {props.children}
      </Show>

      {/* When `fitContent: false`, we wrap image children ssuch that they stretch across the entire image  */}
      <Show when={!props.fitContent && props.khulnasoftBlock?.children?.length}>
        <div
          css={{
            display: 'flex',
