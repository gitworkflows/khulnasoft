...
const createPagesAsync = async (config, createPage, graphql, models, offsets) => {
  ...
  for (let index = 0; index < models.length; index++) {
    ...
    invariant(
      ...
      `@khulnasoft.com/gatsby requires a valid template path for each model`
    );
...
exports.onCreatePage = ({ page, actions }, options) => {
  ...
  if (page.path === '/dev-404-page/' && config.overrideDev404) {
    ...
    // override gatsby's custom 404 dev to allow development on Khulnasoft.com
...
function createSchemaNodeId({ typeName, createNodeId }) {
  return createNodeId(`khulnasoft-gatsby-source-graphql-${typeName}`);
}
