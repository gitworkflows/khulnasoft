...
const KHULNASOFT_SET_STATE_NAME = 'KHULNASOFT_SET_STATE';

const INJECTED_IVM_GLOBAL = 'KHULNASOFT_IVM';
...
var refToProxy = (obj) => {
  ...
  return new Proxy({}, {
    ...
    set(target, key, value) {
        ...
        obj.setSync(key, v);
        ${KHULNASOFT_SET_STATE_NAME}(key, value)
    },
...
const SHOULD_MENTION_INITIALIZE_SCRIPT =
  SDK_NAME === '@khulnasoft.com/sdk-react-nextjs' ||
  SDK_NAME === '@khulnasoft.com/sdk-react' ||
  SDK_NAME === '@khulnasoft.com/sdk-qwik' ||
  SDK_NAME === '@khulnasoft.com/sdk-vue';
...
const getIvm = (): IsolatedVMImport => {
  ...
  const ERROR_MESSAGE = `${MSG_PREFIX}could not import \`isolated-vm\` module for safe script execution on a Node server.
    ...
    For more information, visit https://khulnasoft.com/c/docs/integration-tips#enabling-data-bindings-in-node-environments`;
...
export const runInNode = ({
   ...
   */
  jail.setSync(KHULNASOFT_SET_STATE_NAME, function (key: string, value: any) {
        ...
        ? new ivm.Reference(
            ...
            key === 'khulnasoft'
              ? {
                  ...arg,
                  getUserAttributes: () =>
                    (arg as KhulnasoftGlobals).getUserAttributes(),
                }
              : arg
          )
        : null;
