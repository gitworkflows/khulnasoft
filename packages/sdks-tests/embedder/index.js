...
const server = (req, res) => {
  ...
  fullUrl.searchParams.set('khulnasoft.frameEditing', 'true');
  ...
  <script>
    const handleEventData = (event) => {
      switch (event.data.type) {
        ...
        case 'khulnasoft.sdkInfo':
        case 'khulnasoft.sdkInjected':
    ...
    window.addEventListener('message', (event) => {
      if (event?.data?.type?.startsWith('khulnasoft.')) {
        console.log('KHULNASOFT_EVENT:', event.data.type, handleEventData(event));
      }
    })
  </script>
  <iframe
