# Khulnasoft.com Svelte SDK Changelog (@khulnasoft.com/sdk-svelte)
...
- abe5cba: Feat: exports `setClientUserAttributes` helper that can be used to set and update Khulnasoft's user attributes cookie. This cookie is used by Khulnasoft's Personalization Containers to decide which variant to render.
  ...
  ```ts
  import { setClientUserAttributes } from "@khulnasoft.com/sdk-svelte";
...
- Feature: Added support for mutating Khulnasoft's `state` in custom element events and custom user code.
